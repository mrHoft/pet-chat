 ...
 */
	...
	socket.addEventListener('close', (event) => {
		if (event.wasClean) console.warn(`${CHAT_ID}: WS connection closed`);
		else console.warn(`${CHAT_ID}: WS connection lost`, event.code, event.reason);
	...
	socket.addEventListener('message', (event) => {
		if(event.data[0]=='{' || event.data[0]=='['){
			...
			try { data=JSON.parse(event.data); } catch(err){ console.error('WS message parse', err); }
